...
# The LMS list/detail endpoints require the server-side service key in the
# x-api-key header (same mechanism lms_push.py uses). Resolve it from, in order:
#   1. shell env  LMS_TMS_API_KEY / EXTERNAL_API_KEY_FOR_CLAWDBOT
#   2. project    ./.claude/lms_tms_api_key  or  LMS_TMS_API_KEY=... in ./.claude/.env / ./.env
#   3. user       ~/.claude/lms_tms_api_key  or  LMS_TMS_API_KEY=... in ~/.claude/.env
import os
...
def _resolve_key():
    for name in ("LMS_TMS_API_KEY", "EXTERNAL_API_KEY_FOR_CLAWDBOT"):
        ...
        if v:
            return v
    for p in (".claude/lms_tms_api_key", "~/.claude/lms_tms_api_key"):
        try:
            v = open(os.path.expanduser(p), encoding="utf-8").read().strip()
            if v:
                return v
        except OSError:
            pass
    for p in (".claude/.env", ".env", "~/.claude/.env"):
        v = _key_from_env_file(p)
        if v:
            return v
    return ""
